...
memberDecl
    ...
    |    (modifiers  'class')=> classDeclaration
    |    interfaceDeclaration
    ;
...
importDeclaration
@after{
            log.info(substring( ((CommonToken)$importDeclaration.start).getStartIndex(),
                ((CommonToken)$importDeclaration.stop).getStopIndex())
            );
       }
    :  // 'import'  ('static' )? IDENTIFIER '.' '*' ';'
       'import' ('static' )? IDENTIFIER (options{k=2;}: '.' IDENTIFIER )* ('.' '*' )? ';'
